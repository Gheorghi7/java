...
public class PokerHand {
	...
	public String toString() {
		return "%d. %-16s Rank: %d %-40s Best:%-7s Worst:%-6s %s".formatted(playerNo, score, score.ordinal(), hand, 
				Collections.max(hand, Comparator.comparing(Card::rank)), Collections.min(hand, Comparator.comparing(Card::rank)),(discards.size() >0)?"Discrads: "+discards:"");
	...
	public void setRank(int faceCount) {
		...
		case 2 -> {if(score == Ranking.NONE) score = Ranking.ONE_PAIR ;
		...
		public void evalHand() {
			...
			List<String> duplicateFaceCard = new ArrayList<>();
			faceList.forEach(face -> {
				if(!duplicateFaceCard.contains(face) &&
						Collections.frequency(faceList, face) > 1) {
					duplicateFaceCard.add(face);
				}
				});
			for(String duplicate: duplicateFaceCard) {
