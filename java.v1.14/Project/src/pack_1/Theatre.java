...
public class Theatre {
	...
	public void printSeatMap() {
		String separateLine = "-".repeat(90);
		System.out.print("%1$s%n%2$s Seat Map%n%1$s%n".formatted(separateLine, theatreName));
		int index = 0; 
		for(var s: seats) {
			System.out.print("%-8s%s".formatted(s.seatNum + ((s.reserved) ? "(\u25CF)":""),
	...
	private boolean validate(int count, char first, char last, int min, int max) {
		...
		if(!result) {
			...
			System.out.printf(": Seats mist be between %s and %s%n", seats.first().seatNum, seats.last().seatNum);
		}
		return result;
	}
